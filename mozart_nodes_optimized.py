...
class PetyaClient:
    ...
    def init_session_keys(self, target_key_len):
        ...
        self.public_key_A, self._secret_s = self.engine.generate_keypair()
    ...
    def receive_and_analyze(self, packet_b, sl_seed, use_hacked_key=False):
        ...
        if use_hacked_key:
            print("[КРИТ]: Петя использует дефектный закрытый ключ (ошибка в 1 знак)!")
            seed_to_use = sl_seed + 1

        success, tau, raw_bytes = self.engine.decapsulate_and_verify(
            matrix_A=self.public_key_A,
            packet_b=packet_b,
            secret_s=self._secret_s,
