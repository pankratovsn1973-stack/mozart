...
class MozartReferenceBase(QWidget, BaseControl):
    """ERP-компонент Ссылка. Идентификация строго через calias."""
    ...
    def __init__(self, parent=None):
        ...
        # ✅ ГЛАВНЫЙ БИЗНЕС-ИДЕНТИФИКАТОР КОНТРОЛА
        self._calias = "ref_unknown"
    ...
    @calias.setter
    def calias(self, value):
        self._calias = str(value).strip() if value else "ref_unknown"
    ...
    def init_ui(self):
        ...
        self.label_widget.calias = "lbl_of_ref"  # ✅ Бизнес-идентификатор
        self.label_widget.setObjectName(f"lbl_of_ref_{id(self)}")  # Технический ID для Qt
        ...
        self.edit_runtime.calias = "txt_of_ref"  # ✅ Бизнес-идентификатор
        self.edit_runtime.setObjectName(f"txt_of_ref_{id(self)}")  # Технический ID для Qt
        self.edit_runtime.setReadOnly(True)
        self.layout_assigned.addWidget(self.edit_runtime)

        # --- КНОПКА ВЫБОРА ---
        self.btn_select = QPushButton("📁", self)
        self.btn_select.calias = "btn_select_of_ref"  # ✅ Бизнес-идентификатор
        self.btn_select.setObjectName(f"btn_select_of_ref_{id(self)}")  # Технический ID для Qt
        self.btn_select.setFixedWidth(28)
        self.layout_assigned.addWidget(self.btn_select)

        # --- КНОПКА ОЧИСТКИ ---
        self.btn_clear = QPushButton("✖", self)
        self.btn_clear.calias = "btn_clear_of_ref"  # ✅ Бизнес-идентификатор
        self.btn_clear.setObjectName(f"btn_clear_of_ref_{id(self)}")  # Технический ID для Qt
    ...
    def set_design_mode(self, design_mode, full_path="", calias=""):
        """
        ...
        Args:
            ...
            calias: Собственный алиас элемента (например, "txtBX1").
        """
        ...
        if design_mode and self.layout_assigned:
            ...
            # ✅ ГЕНЕРАЦИЯ УНИКАЛЬНОГО ТЕХНИЧЕСКОГО objectName НА ОСНОВЕ ПУТИ И CALIAS
            unique_qt_name = f"{full_path}.{calias}" if full_path and calias else f"txt_of_ref_{id(self)}"

            # Подменяем безопасной QLabel-заглушкой с внешним видом текстбокса
            self.edit_design = QLabel(self)
            self.edit_design.setObjectName(unique_qt_name)  # <--- Уникальный технический ID для Qt
            self.edit_design.calias = "txt_of_ref"  # <--- ЕДИНСТВЕННЫЙ БИЗНЕС-ИДЕНТИФИКАТОР
